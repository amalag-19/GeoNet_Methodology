...
file_path<-"/storage/home/aua257/work/spill_datasets_run/Cl_spill_whole/"

## Sourcing the modular functions for analysis
source(file = "/storage/home/aua257/work/spill_datasets_run/BaSu_network_v15_modular_functions.R")
...
# load(file = "/storage/home/aua257/work/spill_datasets_run/shape.RData")
# 
...
# ## Analyte preprocessing; Reading and cleaning the Cl dataset
# df_analyte_raw<-read.csv(file = paste0(file_path,"data/Cl_data.csv"),stringsAsFactors = F)
# str(df_analyte_raw)
# 
# ## Preprocessing the analyte dataframe
# df_analyte_preprocessed<-df_analyte_raw[,c(1,2,3)]
# df_analyte_preprocessed$date<-as.Date(df_analyte_raw$Date,"%m/%d/%y")
...
# df_anpoll_preprocessed[(nrow(df_anpoll_preprocessed)-45):nrow(df_anpoll_preprocessed),]
# 
# ## saving this dataframe
...
# projected_nodeIDs_list<-projected_nodeIDs_list_generator(file_path=file_path)
...
#   flow_dist_from_list_projected<-unlist(x = flow_dist_from_list_projected_combined,recursive = F)
...
spill_data_processed<-read.csv(file = "/storage/home/aua257/work/spill_datasets_run/Spill_data_processed.csv",stringsAsFactors = F)
...
for (j in 1:nrow(df_threshold_dist_km)){
  ...
  for(i in 1:nrow(polluter_test_matrix)){
    ...
    polluter_test_matrix[i,9]<-as.numeric(test_result[[3]][9])
    ## Storing the p values and test indicator for downstream temporal test
    if(as.numeric(test_result[[2]][12])==1){
      polluter_test_matrix[i,10]<-as.numeric(test_result[[2]][10])
      polluter_test_matrix[i,11]<-as.numeric(test_result[[2]][11])
    }
    polluter_test_matrix[i,12]<-as.numeric(test_result[[3]][12])
    ## Storing the p values and test indicator for final upstream vs. downstream spatio-temporal test
    if(as.numeric(test_result[[3]][15])==1){
      polluter_test_matrix[i,13]<-as.numeric(test_result[[3]][13])
      polluter_test_matrix[i,14]<-as.numeric(test_result[[3]][14])
    }
    polluter_test_matrix[i,15]<-as.numeric(test_result[[3]][15])
